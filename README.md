...
[English Documentation](README_EN.md) | [功能特性](#功能特性) | [快速开始](#快速开始) | [MCP工具](#mcp工具) | [云函数架构](docs/CLOUDFUNCTION_ARCHITECTURE.md)
...
- 🎨 **现代UI** - 简洁直观的界面设计
- ⚙️ **灵活配置** - 支持多种AI服务配置
- ☁️ **云函数支持** - 模块化云函数架构，支持数据存储和业务扩展（测试实现）
...
│   └── app.ts              # 应用入口
├── docs/                   # 文档目录
│   ├── CLOUDFUNCTION_ARCHITECTURE.md  # 云函数架构文档
│   └── ...                 # 其他文档
└── README.md               # 项目说明
```
...
- **云函数系统**：提供云端数据存储和业务逻辑处理（测试实现）
- **MCP工具系统**：支持多种实用工具的智能调用
- **组件系统**：可复用的UI组件库
- **服务层**：AI服务、云函数调用、数据存储等核心服务
...
4. 部署云函数（可选，测试版本）：
   - 在微信开发者工具中右键 `cloudfunctions/supchat` 目录
   - 选择"上传并部署：云端安装依赖"
   - 详细说明请参考 [云函数架构文档](docs/CLOUDFUNCTION_ARCHITECTURE.md)
   - **注意**：当前为测试实现，返回模拟数据，主要用于验证架构设计
