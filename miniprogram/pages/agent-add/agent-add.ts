// agent-add.ts
import { AgentInput, MCPConfig } from '../../lib/types/agent'
...
Page({
  ...
  async onSave() {
    ...
    try {
      ...
      // 创建配置
      const config = AgentConfigStorage.createConfigFromInput(agentInput)
      ...
      if (success) {
        ...
        setTimeout(() => {
          wx.redirectTo({
            url: `/pages/agent-detail/agent-detail?id=${config.id}`
          })
        }, 1500)
      } else {
        wx.showToast({
